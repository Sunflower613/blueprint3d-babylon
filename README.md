...
*   🪞 **多级镜面与反射方案 (Multi-Level Reflection Strategy)**：
    *   **高精度镜面（浴室镜、全身镜）**：采用**独立物体 + MirrorTexture** 实现 100% 真实精准反射玩家和面前的物品，避免环境纹理（探针）无法精准对齐的问题。
    *   **氛围感反射材质（粗糙表面等）**：采用**反射探针（区域环境纹理）**提供“有亮光、能倒影出房间大概轮廓”的氛围感，以极好的运行性能实现像素级近似的观感。
    *   **低频反光组件（玻璃窗、金属把手）**：采用静态或低频更新的通用 **CubeMap 环境纹理** 满足微弱反光质感，实现极致渲染性能。
